...
function drawTrend(series){const values=series.map(x=>Number(x.value||0)),max=100,left=30,top=20,width=720,height=190;document.querySelector('#trendEmpty').style.display=values.length?'none':'block';const points=values.map((v,i)=>`${left+(values.length===1?width/2:i*width/(values.length-1))},${top+height-(Math.max(0,Math.min(max,v))/max)*height}`).join(' ');document.querySelector('#linePath').setAttribute('d',points?`M ${points}`:'');document.querySelector('#areaPath').setAttribute('d',points?`M ${left} ${top+height} L ${points} L ${left+width} ${top+height} Z`:'');}
 ...
 function renderZoneBoundaries(hotspots){if(!zoneBoundaryLayer||!knownZones.length)return;zoneBoundaryLayer.clearLayers();const hotspotMap={};hotspots.forEach(item=>{hotspotMap[String(item.zone)]={hotspot_score:Number(item.hotspot_score||0)};});const minLat=40.68,minLon=-74.02,latStep=.0138,lonStep=.007,columns=20;knownZones.forEach(zone=>{const index=Math.max(0,Number(zone)-4),row=Math.floor(index/columns),column=index%columns,lat=minLat+row*latStep,lon=minLon+column*lonStep,hotspot=hotspotMap[String(zone)],polygon=L.polygon([[lat,lon],[lat+latStep,lon],[lat+latStep,lon+lonStep],[lat,lon+lonStep]],{color:hotspot?'#ff7f8e':'#436181',weight:hotspot?2:.7,fillColor:hotspot?'#ff7f8e':'#1c3858',fillOpacity:hotspot?.22:.035,interactive:true});polygon.bindTooltip(`Khu ${esc(zone)}${hotspot?` · Điểm nóng · ${hotspot.hotspot_score.toFixed(0)}/100`:''}`,{sticky:true,direction:'top'});polygon.addTo(zoneBoundaryLayer);});}
